<template>
	<div class="container">
		<div class="inner">
				...
				<article class="area_list">
					<div class="colb">
						...
						<div class="right">
							...
							<div class="bx_search type1">
								<input type="text" class="ipt" placeholder="검색어를 입력하세요." v-model="searchWrd" v-on:keyup.enter="fn_Search_Input"><button type="button" v-on:click="fn_Search_Input">검색</button>
							</div>
						</div>
					</div>
					<table class="tbl">
						...
						<tbody v-else>
							<tr v-for="(stock,i) in stockList" v-bind:key="i" v-on:click=fnStockDetail(stock.isuSrtCd,stock.corp.bizno,$event)>
            ...
            <button v-on:click="fn_Get_Stock_List(0)" ref="btn_first" >&lt;&lt;</button>
            ...
            <button v-on:click="fn_Get_Stock_List(pageTotal-1)" ref="btn_last">&gt;&gt;</button>
					</div>
				</article>
			</section>
		</div>
...
export default {
    ...
    fn_Search_Input() {
      ...
      if(pageNo == 0) {
        this.$refs.btn_first.hidden = true;
        this.$refs.btn_last.hidden = false;
      } else if(pageNo == this.pageTotal-1){
        this.$refs.btn_first.hidden = false;
        this.$refs.btn_last.hidden = true;
      } else if(pageNo > 0 && pageNo < this.pageTotal) {
        this.$refs.btn_first.hidden = false
        this.$refs.btn_last.hidden = false;
      }
			...
			if (event) {
                ...
                /*
								for (var idx in results) {
									var tempDate = new Date();
									if(results[idx].inddTm.startsWith("9")) {
										tempDate.setUTCHours(parseInt(results[idx].inddTm.substring(0,1)));
										tempDate.setUTCMinutes(parseInt(results[idx].inddTm.substring(1,3)));
										tempDate.setUTCSeconds(parseInt(results[idx].inddTm.substring(3,5)));
										tempDate.setUTCMilliseconds(0);
									} else {
										tempDate.setUTCHours(parseInt(results[idx].inddTm.substring(0,2)));
										tempDate.setUTCMinutes(parseInt(results[idx].inddTm.substring(2,4)));
										tempDate.setUTCSeconds(parseInt(results[idx].inddTm.substring(4,6)));
										tempDate.setUTCMilliseconds(0);
									}
...
*/
    ...
    fnStockDetail(isuSrtCd, bizno, e) {
      sessionStorage.setItem("detailBizNo", bizno);
      sessionStorage.setItem("detailIsuSrtCd", isuSrtCd);

      if(e.target.localName.indexOf('td') > -1) {
        this.$api.fnGetApiCall('https://c-api.10bagger.co.kr/v1/report/outline?bizNo='+bizno+'&isuSrtCd='+isuSrtCd)
        .then((res) =>{
          var json = res.data;
          console.log(json)
          var themes = [];
          var price ,upDown, prevPrice;
          for(var idx in json.themes) {
            themes.push(json.themes[idx].themeName);
          }
          ...
          this.$api.fnGetApiCall('/sectorchart/stockHistory?reqCnt=1&trnsmCycleTpCd=D&inqStrtDd='+today+'&inqEndDd='+today+'&issuecode='+json.isuSrtCd+'&marketcode='+json.mkt.toLowerCase())
          .then((resPrice) => {
            console.log(resPrice.data)
            var jsonPrice = resPrice.data.hisLists[0];

            console.log(jsonPrice)

            var cmpprevddTpCd = jsonPrice.cmpprevddTpCd;
            var cmpprevddTpPrct = jsonPrice.cmpprevddTpPrct;
            price = jsonPrice.trdPrc;
            prevPrice = jsonPrice.cmpprevddPrc;
            if(cmpprevddTpCd == '1' || cmpprevddTpCd == '2' || cmpprevddTpCd == '6' || cmpprevddTpCd == '7') {
              upDown = 'up';
            }else if(cmpprevddTpCd == '4' || cmpprevddTpCd == '5' ||cmpprevddTpCd == '8' ||cmpprevddTpCd == '9') {
              upDown = 'down';
            }else {
              upDown = '';
            }

            if(cmpprevddTpPrct > 0) {
              cmpprevddTpPrct = +"+"+cmpprevddTpPrct+"%"
            } else{
              cmpprevddTpPrct = cmpprevddTpPrct+"%"
            }
            const now = this.$getCurrentDate();
            this.$store.dispatch('setTopData', {
              type:json.mktCapScale,
              tit:json.isuKorNm,
              num:json.isuSrtCd,
              stock:json.mkt,
              tag:themes,
              ...
              increase:{
                nm:upDown,
                price:[prevPrice, cmpprevddTpPrct]
              },
              date:now.substr(0,4) + '/'+now.substr(4,2)+ '/'+now.substr(6,2) + ' '+now.substr(8,2)+':'+now.substr(10,2),
              desc:"전체상장사 "+json.rankTssMax+"종목 중 기술력(TSS)순위 "+json.rankTss+"위",
              onWish:"on",
              onReco:"on",
              stock_index:[170.50, 170, 170, 370, 100],
            });
          })
          .catch((catchPrice) =>{
        ...
        this.$router.push({ path: '/detailStock'});
      } else if(e.target.localName.indexOf('button') > -1) {
        window.open("/report/stockReport?bizNo="+bizno+"&isuSrtCd="+isuSrtCd, "", "width=1180,height=810");
        //this.$router.push({ path: '/reportstock', query: { isuSrtCd: isuSrtCd, bizno:bizno }});
      }
    },
    fnGetReport(e){
      console.log(e)
